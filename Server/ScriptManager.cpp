...
void ScriptManager::runScript(std::string functionName, int arg_count, ...)
{
	if (this->scriptLoaded)
	{
		...
		for (int i = 0; i < arg_count; i++)
		{
			...
			if (var->_type == variant::Type::TYPE_GAMEOBJECTPTR) {
				dukglue_push(this->ctx, var->_asGameObjectPtr);
			}
			else if (var->_type == variant::Type::TYPE_FLOAT) {
				dukglue_push(this->ctx, var->_asFloat);
			}
			else if (var->_type == variant::Type::TYPE_PLAYERPTR) {
				dukglue_push(this->ctx, var->_asPlayerPtr);
			}
